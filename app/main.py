...
from app.config import Settings, chain_env, load_wallets
    ...
    async def run() -> None:
        for chain, rpc in clients(settings).items():
            try:
                ...
                console.print(f"[green]✓[/green] {chain}: chain_id={result['chain_id']} latest_block={result['latest_block']}")
...
def run() -> None:
    ...
    async def consume(chain: str) -> None:
        async for activity in watch_chain(chain, wallets, rpc_by_chain[chain], settings.poll_interval_seconds):
