...
def _classify(
    ...
    if analysis.asset_in and (analysis.quote_out or native_value > 0):
        ...
        analysis.reason = "钱包卖出非报价 Token，V2 路径最终输出报价 Token"
        analysis.action_token = analysis.asset_out[0].token
        ...
        analysis.reason = "交易支付原生 BNB，V2 路径最终输出非报价 Token"
        ...
        analysis.reason = "V2 路径从报价 Token 转为非报价 Token"
        analysis.action_token = route_output_assets[-1]
        ...
        analysis.reason = "V2 路径从非报价 Token 转为报价 Token"
        ...
        analysis.reason = "检测到 V2 Pair Swap，但无法确定完整报价方向"
...
async def enrich_bsc_analysis(
    ...
    for swap, pair in zip(analysis.pair_swaps, pair_metadata, strict=False):
        ...
        swap.protocol = factory.name if factory else "V2-compatible DEX"
